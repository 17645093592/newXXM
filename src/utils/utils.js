...
export default class utils {
    ...
    static setStorage(key, value) {
        ...
        // 数组/对象类型保存时，需要转json
        window[global.STORAGE].setItem(key, typeof value == 'object' ? JSON.stringify(value) : value);
    }
    /**
     * 读取缓存
     * @param {String} *key 缓存的key
    ...
    static getStorage(key) {
        if (this.checkEmpty(key)) return '';
        let value = window[global.STORAGE].getItem(key);
    ...
    static getObjParam(obj, type, index = 0) {
        ...
        return type == 'key' ? Object.keys(obj)[index] : obj[Object.keys(obj)[index]];
    }
    // ------------------------------验证信息-------------------------------------
    /**
     * 判断值是否为空
    ...
    // 网站是否正确---'网站不正确'返回true
    static checkWeb(str = '') {
        let reg = new RegExp('^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/|www\.)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+$');
        if (!reg.test(str)) return true;
    };
    // 只能输入浮点数并保存小数
    static checkFloatNumber(floatNum, point) {
        ...
        newNum = newNum.replace('.', '$#$').replace(/\./g, '').replace('$#$', '.');
        // 保留n位小数
        if (newNum.includes('.')) {
            return floatNum != newNum.substring(0, newNum.indexOf('.') + Number(point) + 1);
        }
    ...
    static checkIDcard(card = '') {
        ...
        if (new Date(birthday).getDate() != birthday.substring(birthday.length - 2)) return true;
    };
    // 获取身份证中的生日---传入通过验证的身份证号
    static getBirthday(card) {
        // 15位的身份证需要加一个'19'
        let birthday = card.length == 18 ? card.substr(6, 8) : '19' + card.substr(6, 6);
        // 返回格式'yyyy-mm-dd'
    ...
    static dateChange(timestamp = '', symbol = '-') {
        ...
        let month = this.add0(oDate.getMonth() + 1);
        let day = this.add0(oDate.getDate());
    ...
    static areaChange(code, num = 3) {
        let areaJson = this.returnCode('areaJson');
        ...
        let specialCityCode = code.substr(0, 4) + 'XX';
        // 循环查找 市
    ...
    static codeChange(code, key, jsonName = 'codeJson') {
        ...
        let codeList = this.returnCode(jsonName)[key];
        // 循环查找码表的中文
        if (!this.checkEmpty(codeList)) {
            for (let val of codeList) {
    ...
    static confirm(msg, cb) {
        Vue.prototype.$confirm(msg, global.PRODUCT, {
    ...
    */
    static resourceChange(list, id, pid) {
        ...
        let map = {};
        let newList = this.cloneObj(list);
